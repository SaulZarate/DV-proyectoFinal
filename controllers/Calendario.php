...
class Calendario{
    ...
    public static function getAllEventsByUser($idUser, $desde, $hasta, $filter = ""){
        $eventos = DB::getAll("SELECT e.*, u.* FROM eventos e, usuarios u WHERE e.eliminado = 0 AND e.idUsuario = u.idUsuario AND u.idUsuario = {$idUser} AND DATE('{$desde}') <= DATE(e.fechaInicio) AND DATE(e.fechaInicio) <= DATE('{$hasta}') {$filter} ORDER BY e.fechaInicio ASC");
        return $eventos;
    }
